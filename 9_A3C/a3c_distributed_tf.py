...
class ACNet(object):
    ...
    def __init__(self, scope, opt_a=None, opt_c=None, global_net=None):
        if scope == 'global_net': # get global network
            with tf.variable_scope(scope):
                ...
                self.a_params, self.c_params = self._build_net(scope)[-2:] # 返回值的后两个
        else:
            with tf.variable_scope(scope):
                self.s = tf.placeholder(tf.float32, [None, N_S], 'S')
                ...
                with tf.name_scope('a_loss'):
                    log_prob = tf.reduce_sum(
                        tf.log(self.a_prob) * tf.one_hot(self.a_his, N_A, dtype=tf.float32), # N_A = env.action_space.n
    ...
    def _build_net(self, scope):
        ...
        with tf.variable_scope('actor'):
            ...
            a_prob = tf.layers.dense(l_a, N_A, tf.nn.softmax, kernel_initializer=w_init, name='ap') # N_A = env.action_space.n
        with tf.variable_scope('critic'):
            l_c = tf.layers.dense(self.s, 100, tf.nn.relu6, kernel_initializer=w_init, name='lc')
            v = tf.layers.dense(l_c, 1, kernel_initializer=w_init, name='v')  # state value
        a_params = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope=scope+'/actor') # 获取actor网络中的参数
